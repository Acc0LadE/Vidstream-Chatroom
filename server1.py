...
def showinf():
    ...
    showinfor.grab_set()
    _INFO = Label(showinfor,text='This is a VoIP(Voice over Internet Protocol) application \n based on chatting and camera streaming developed \n for  ease of communication among groups of people \n for various purposes like gaming, meetings and study.',font=('',12),relief=FLAT,bg='green2',fg='black')
    _INFO.place(x=35,y=15)
...
def video(): 
    ...
    def start_screenshare():
        reciever = StreamingServer('192.168.29.232', 8888)
    ...
    def hear():
        receiver=AudioReceiver('192.168.29.232',9999)
        receiver_thread=threading.Thread(target=receiver.start_server)
    ...
    btn_listen = Button(window, text="Start Listening", width=50,command=listen)
    ...
    btn_audio = Button(window, text="Start speaking", width=50,command=hear)
